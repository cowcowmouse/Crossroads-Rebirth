...
func setup(id: String, portrait_path: String):
	...
	if portrait_sprite and ResourceLoader.exists(portrait_path):
		...
		portrait_sprite.scale = Vector2(0.65, 0.65)   # 控制人物大小
		print("✅ 立绘加载成功：", portrait_path)
	else:
		print("❌ 立绘加载失败")
...
func _on_input_event(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if dialogue_scene:
			...
			# 调用新版本的 show_dialogue（不再传 lines，由 DialoguePanel 内部生成随机对话）
			panel.show_dialogue(member_id, "res://project/assets/character/fullbody/" + member_id + ".png")
			print("✅ 对话面板已创建")
		else:
			print("❌ 找不到 DialoguePanel.tscn")
